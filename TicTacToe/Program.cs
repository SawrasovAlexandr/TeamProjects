...
while (userWin < 3 && pazzleWin < 3) //выполнять пока кем то не будет набрано 3 победы
{
    ...
    if (Win(gameCells))
    {
        Console.WriteLine(tab + (pazzleTurn ? "   Пазл выиграл!" : "    Ты выиграл!")); //если выпал выигрыш, Пазл либо хвастается либо поздраляет
        if (pazzleTurn) pazzleWin++;
        else userWin++;
    }
    ...
    if (userWin == 2 && userWin == pazzleWin) whoWins = !whoWins;
    else whoWins = (userWin <= pazzleWin);
    Console.WriteLine();
